...
describe.only("Manufacturer Contract", function () {
  let owner: any;
  let user1: any;
  let user2: any;
  let ManufacturerContract: any;
  let OzContract: any;
  let OzContract2: any;
  ...
  before(async function () {
    ...
    const ManufacturerContract_ = await ethers.getContractFactory(
      "Manufacturer"
    );
    ManufacturerContract = await ManufacturerContract_.deploy();
    await ManufacturerContract.deployed();
  });
  describe("Admins", async function () {
    it("should have one admin", async function () {
      const adminCount = await ManufacturerContract.getAdminCount();
      await expect(adminCount).to.equal(1);
    });
    it("should have added an admin", async function () {
      // eslint-disable-next-line prettier/prettier
      const tx = await ManufacturerContract.connect(owner).addAdmin(user1.address);
      await tx.wait();
      const adminCount = await ManufacturerContract.getAdminCount();
  ...
  describe("contracts", async function () {
    ...
    it("should add that NFT contract to the Manufacturer's db", async function () {
      const contractAddress = OzContract.address;
      const tx2 = await ManufacturerContract.addContract(contractAddress);
      await tx2.wait();
      const contractCount = await ManufacturerContract.getContractCount();
    ...
    it("should add the second NFT contract to the Manufacturer's db", async function () {
      const contract2Address = OzContract2.address;
      const tx3 = await ManufacturerContract.addContract(contract2Address);
      await tx3.wait();
      const contractCount = await ManufacturerContract.getContractCount();
