...
async function main() {
  ...
  */
  ...
  // Verify the contract
  const waitFor = (delay: number) => new Promise(resolve => setTimeout(() => {
    hre.run("verify:verify", {
      address: deployedCollectionContract.address,
      constructorArguments: [COLLECTION_NAME, COLLECTION_SYMBOL, COLLECTION_MAX_NUM_OF_ITEMS,
        COLLECTION_PRODUCT_NAME, COLLECTION_MAKER_ADDRESS],
    })
  }, delay));
