...
export default function Home(): NextPage {
  ...
  async function loadNFTs() {
    ...
    // const provider = new ethers.providers.JsonRpcProvider()
    // let contract = new ethers.Contract(marketplaceAddress, NFTMarketplaceValue.abi, provider) as NFTMarketplace;
  ...
  async function buyNft(nft: never) {
    /* needs the user to sign the transaction, so will use Web3Provider and sign it */
    const web3Modal = new Web3Modal()
    const connection = await web3Modal.connect()
    const provider = new ethers.providers.Web3Provider(connection)
    const signer = provider.getSigner()
    const contract = new ethers.Contract(marketplaceAddress, NFTMarketplaceValue.abi, signer) as NFTMarketplace;
